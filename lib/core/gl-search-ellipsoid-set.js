...
export class GlSearchEllipsoidSet extends GlLayer {
  constructor(params, fromJSON) {
    super();
    this.isGlSearchEllipsoidSet = true;
    this.type = 'GlSearchEllipsoidSet';
    this.name = '';
  ...
  addChild(mesh, params) {
    if (!mesh.isGlSearchEllipsoid) return;
    params = params || {};
  ...
  // -------------------------------------
  // makes GlSearchEllipsoidSet's properties
  // default to children.
  ...
  getBoundingBox(includeAll) {
    const meshEllipsoidSetBB = new Box3();
    ...
    for (const object of this.children) {
      ...
      if (object.getBoundingBox && !skip) {
        ...
        if (objectBB) {
          ...
          meshEllipsoidSetBB.expandByPoint(min);
          meshEllipsoidSetBB.expandByPoint(max);
        }
      }
    }
    if (!meshEllipsoidSetBB.isEmpty()) {
      meshEllipsoidSetBB.getSize(this.bbox);
      if (this.bbox.length() < 1) this.bbox.set(1, 1, 1);
    }
    return meshEllipsoidSetBB.isEmpty() ? null : meshEllipsoidSetBB;
  }
  ...
  toJSON(meta, keepUuid = false) {
    ...
    if (isRootObject) {
      output.metadata = {
        version: 5.0,
        type: 'GlSearchEllipsoidSet',
        generator: 'GlSearchEllipsoidSet.toJSON'
      };
    }
    ...
    if (this.children.length) {
      object.ch = [];
      for (let i = 0; i < this.children.length; i++) {
        if (this.children[i].isGlSearchEllipsoid) {
