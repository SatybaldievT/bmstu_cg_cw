...
class Text extends Mesh {
  ...
  async sync(callback) {
    if (this._needsSync) {
      ...
      if (this._isSyncing) {
        ...
        const textRenderInfo = await getTextRenderInfo({
          text: this.text,
          font: this.font,
  ...
  _prepareForRender(material, orientMatrix, camera) {
    ...
    if (textInfo) {
      ...
      uniforms.uTroikaUsePerpendicularTo.value = this.usePerpendicularTo ? true : false;
      ...
      if (clipRect && Array.isArray(clipRect) && clipRect.length === 4) {
        ...
        const pad = (this.fontSize || 0.12) * 100
