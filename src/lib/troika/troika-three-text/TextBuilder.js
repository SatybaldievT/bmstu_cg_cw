 ...
 */
async function getTextRenderInfo(args, callback) {
  hasRequested = true
  args = assign({}, args)
  ...
  const e = await typesetInWorker(args);
  ...
  if (neededHeight > currentHeight) {
    ...
    // As of Three r136 textures cannot be resized once they're allocated
    // on the GPU, we must dispose to reallocate it
    sdfTexture.dispose()
  }
  ...
  await Promise.all(neededSDFs.map(glyphInfo =>
